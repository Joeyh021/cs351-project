...
A companion object is used for the clock divider to enable functional module creation. Companion objects in Scala are objects declared in the same file as the class of the same name, and allow to add static methods and factory methods to the class. The \txt{apply} method is a special method in Scala that is used when calling an object like a method: \txt{Foo(bar)} desugars to \txt{Foo.apply(bar)}.
