...
\begin{itemize}
    ...
    \item \textit{\textbf{SHOULD}} be able to flexibly map inputs and outputs to physical device pins
    \item \textit{\textbf{COULD}} be able to run PIO programs originally written for the RP2040
\end{itemize}

A secondary objective of the implementation of the PIO device is to contribute back to the the open source hardware and Chisel communities by providing an open source implementation of PIO and the PIO assembly language. The project is worked on and documented in a public GitHub repository\footnote{\url{https://github.com/Joeyh021/riscv-pio}} where it shall remain after the completion of the project for others to use within their own SoC designs.
...
All implementation work targets FPGAs, specifically a Xilinx Artix 7 device (XC7A100T-1CSG324C) as included on the Digilent Nexys A7-100T board. This provides us with an ideal platform for rapid prototyping and testing with which we are already familiar.
